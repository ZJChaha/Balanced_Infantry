...
void USART1_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN USART1_IRQn 1 */
    ...
    if((tmp_flag != RESET))
    {
        ...
        usart1_data_process(rx_buffer1,rx_len1);

        HAL_UART_Receive_DMA(&huart1,rx_buffer1,100);
    }
...
void UART5_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN UART5_IRQn 1 */
    ...
    if((tmp_flag != RESET))
    {
        ...
        rx_len5 =18-temp;
        if(rx_len5==18) {
            sbus_to_rc(rx_buffer5, &rc_ctrl);
        }
        HAL_UART_Receive_DMA(&huart5,rx_buffer5,18);
    }
...
void UART7_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN UART7_IRQn 1 */
    ...
    uint8_t tmp_flag =__HAL_UART_GET_FLAG(&huart7,UART_FLAG_IDLE);
    if((tmp_flag != RESET))
    {
        __HAL_UART_CLEAR_IDLEFLAG(&huart7);
        HAL_UART_DMAStop(&huart7);
        uint16_t temp=__HAL_DMA_GET_COUNTER(&hdma_uart7_rx);
        rx_len7 =500-temp;
        uart7_data_process(rx_buffer7,rx_len7);

        for (int i = 0; i < 500; i++) {
            rx_buffer7[i]=0;
        }
        HAL_UART_Receive_DMA(&huart7,rx_buffer7,500);
    }
...
void USART10_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN USART10_IRQn 1 */
    uint8_t tmp_flag =__HAL_UART_GET_FLAG(&huart10,UART_FLAG_IDLE);
    if((tmp_flag != RESET))
    {
        ...
        HAL_UART_Receive_DMA(&huart10,rx_buffer10,100);
    }
